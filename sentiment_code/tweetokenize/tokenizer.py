...
# Function added:
# 1) Emoticon sentiment analysis
# 2) Emoji sentiment analysis
# 3) Error Handling: If some words in the source text cannot be transferred to unicode, ignore them.
#
# - - - - - - - - - - - - - - - Original Author - - - - - - - - - - - - - - - -
...
##############################  Sentiment value of emoji and emoticon  ##########################
global emoji_value
emoji_value = 0
##############################  Emoticon further sentiment analysis  ############################
# Emoticons
emoticons_list = \
	[
        # Positive emoticons
        [
                u':-)', u':)', u':o)', u':]', u':3', u':c)', u':>', u'=]', u'8)', u'=)', u':}', u':^)', u':っ)',u'(:',
                u':-D', u'8-D', u'8D', u'x-D', u'xD', u'X-D', u'XD', u'=-D', u'=D', u'=-3', u'=3', u'B^D',u':D',
                u':-))',
                u'>:P', u':-P', u':P', u'X-P', u'x-p', u':-p', u':p', u':-Þ', u':Þ', u':þ', u':-þ', u':-b',
                u';-)', u';)', u'*-)', u'*)', u';-]', u';]', u';D', u';^)', u':-,', u'(;',
                u':*', u':^*',
                u':\'-)', u':\')',
                u'O:-)', u'0:-3', u'0:3', u'0:-)', u'0:)', u'0;^)',
                u'>:)', u'>;)', u'>:-)',
                u'}:-)', u'}:)', u'3:-)', u'3:)',
                u'o/\\o',u'\\o/'
                u'*<|:-)',
        ...
        # Negative emoticons
		[
                u'>:[', u':-(', u':(', u':-c', u':c', u':-<', u':っC', u':<', u':-[', u':[', u':{',
                u':-||', u':@', u'>:(', u':-@',
                u':\'-(', u':\'(',
                u':-###..', u':###..',
                u'>_<'
                u'D:<', u'D8', u'D;', u'v.v', u'D-\':',
                u'</3'
        ]
        # The rest will be recognized as neutral
	]

def determineSentimentFromEmoticons(emoticon):
    if emoticon in emoticons_list[0]:
        return 1
    elif emoticon in emoticons_list[1]:
        return -1
    else:
        return 0
##############################  Emoji further sentiment analysis  ############################
EMOJI_DATA = 'emoji_values.txt'
def getEmojiSentiment(in_file):
    fp = open(in_file , 'r')
    emoji_result = {}
    cnt = 1
    for lines in fp.readlines():
        line = lines.replace("\n", "")
        if cnt == 3:
            id = unicode(line, encoding='utf-8')
        elif cnt == 11:
            score = line
        elif cnt == 12:
            adjust = line
        elif cnt == 13:
            emoji_result[id] = {"score": score, "adjust": adjust}
            cnt = 0
        cnt += 1
    return emoji_result

emoji_values = getEmojiSentiment(EMOJI_DATA)

def determineSentimentFromEmoji(emoji):
    dic = emoji_values[emoji]
    score = float(dic["score"])
    adjust = float(dic["adjust"])
    if score >= 0.35 or (score >=0.2 and adjust < 0.2):
        # print "%d || %f || %f" % (1,score,adjust)
        return 1
    elif score <= -0.35 or (score <=-0.2 and adjust < 0.2):
        # print"%d || %f || %f" % (-1,score,adjust)
        return -1
    else:
        # print"%d || %f || %f" % (0, score, adjust)
        return 0
...
def _unicode(word):
    if isinstance(word, unicode):
        return word
    try:
        word_uni = unicode(word, encoding='utf-8')
    except:
        word_uni = u''
    return word_uni
...
### Modified ###
def _isemoji(s):
    result = len(s) == len(u'\U0001f4a9') and any(l <= s <= u for l, u in emoji_ranges) or s in emoji_flags
    if result:
        globals()['emoji_value'] += determineSentimentFromEmoji(s)
    return result
...
class Tokenizer(object):
    ...
    def _separate_emoticons_punctuation(self, word):
        ...
        ### Modified ### (In fact, new added function)
        def _isemoticon(emoticon):
            is_emoticon = emoticon in self._emoticons
            if is_emoticon:
                globals()['emoji_value'] += determineSentimentFromEmoticons(emoticon)
            return is_emoticon

        while i < len(word):
            # greedily check for emoticons in this word
            for l in range(self._maxlenemo, 0, -1):
                if _isemoticon(word[i:i+l]) or _isemoji(word[i:i+l]):
    ...
    def tokenize(self, message):
        ...
        return (message, emoji_value)  ### Modified ###
